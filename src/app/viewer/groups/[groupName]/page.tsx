...
export default function ViewerGroupPage({ params }: GroupPageProps) {
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="w-80 bg-white border-r border-slate-200 flex flex-col">
        <div className="flex-1 overflow-hidden">
          <ViewerPostSidebar posts={groupPosts} />
        </div>
      </div>
        ...
        <main className="flex-1 bg-slate-50 text-slate-900">
          <div className="max-w-4xl px-6 py-12">
            ...
            <div className="mb-8 border-b border-slate-100 pb-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-400 mb-2">Group Videos</p>
              <h1 className="text-3xl font-extrabold text-slate-900">{groupName}</h1>
              <p className="text-sm text-slate-600 leading-relaxed mt-2">
                Browse all videos in this group.
              </p>
            </div>
            ...
            <div>
              {groupPosts.length === 0 ? (
                <div className="rounded-lg border border-slate-100 bg-white p-6 text-center">
                  <p className="text-sm text-slate-600">No videos found in this group.</p>
                </div>
              ) : (
                  ...
                  {groupPosts.map((post) => (
                    ...
                    >
                      <div className="flex-1 flex items-center gap-3">
                        ...
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider whitespace-nowrap ${getTagBadgeClass(post.tag)}`}>
                          {formatTag(post.tag)}
                        </span>
                      </div>
